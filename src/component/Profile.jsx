...
const isAuthor = UserId && UserData && UserId === UserData._id ? true : false;
  useEffect(() => {
    if(UserId) {
      ...
      const response = await axios.post(`http://localhost:5000/api/v1/users/getUserChannelProfile`, {
        userId: UserId
      })
     ...
     const fetchPosts = async() => {
      const response = await axios.post(`http://localhost:5000/api/v1/posts/getUserPost`, {
            userId: UserId
      })
