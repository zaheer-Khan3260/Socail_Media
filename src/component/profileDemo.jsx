...
import appwriteServices from "../appwrite/databaseSevices"
...
const isAuthor = UserId && UserData && UserId === UserData._id ? true : false;
  ...
  useEffect(() => {
    if(UserId) {
     ...
     const response= await axios.post("/api/v1/users/getUserById", UserId)
     if(response.data) {
      await axios.get(`/api/v1/users/c/${response.data.username}`).then((profileData) => {
          setProfile(profileData)
      })
     }}

     const fetchPosts = async() => {
      const response = await axios.post(`/api/v1/posts/c/${UserId}`).then((posts) => {
          setPosts(posts)
      })
     }
    fetchData()
         ...
         post.UserId === UserId ? 
          ...
          className='min-[375px]:h-36 min-[375px]:w-[5.8rem] min-[425px]:w-[6.6rem]
            min-[320px]:h-28 min-[320px]:w-[4.9rem] min-[950px]:w-[8.2rem] min-[950px]:h-48 bg-black mb-1 flex items-center justify-center'>
                <img src={appwriteServices.getFilePreview(post.featuredImage)} alt="" 
                className='h-full'
                />
          </div> 
          :null
       ...
       posts.map((post) => (
         post.UserId === UserId ? 
          ...
          className='ml-1 min-[950px]:w-full min-[950px]:h-48 bg-black mb-1 flex items-center justify-center'>
                <img src={appwriteServices.getFilePreview(post.featuredImage)}  className='h-full ' alt=""/>
          </div> 
          :null
